...
def plot_imem_iv_tee(df):
    # Convert the 'Date' column to datetime format
    df_plot = df[(df["imem_ecc"] < 38158950)]
    df_plot["timestamp"] = pd.to_datetime(df_plot["timestamp"])

    df_plot["ratio_iv"] = df_plot["iv"] / df_plot["imem_ecc"]
    df_plot.loc[(df_plot["iv"] == 0) & (df_plot["imem_ecc"] == 0), "ratio_iv"] = 1
